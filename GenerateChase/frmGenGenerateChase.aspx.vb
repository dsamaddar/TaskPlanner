...
Partial Class GenerateChase_frmGenGenerateChase
    ...
    Protected Sub drpChaseList_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles drpChaseList.SelectedIndexChanged
        Try
            ...
            Else
                ...
                GetChaseInstruction(drpChaseList.SelectedValue)
            End If
    ...
    Protected Sub GetChaseInstruction(ByVal ChaseDefinitionID As String)
        Dim ChaseDef As clsChaseDefinition = ChaseDefData.fnGetChaseInstruction(ChaseDefinitionID)

        Dim Exists As Boolean = False
        drpIntRepList.Enabled = True
        For Each itm As ListItem In drpIntRepList.Items
            If itm.Value = ChaseDef.PrimarySupportRep Then
                Exists = True
                Exit For
            End If
        Next
        ...
        If Exists = True Then
            drpIntRepList.SelectedValue = ChaseDef.PrimarySupportRep
        End If

        lblChaseInstruction.Text = ChaseDef.ChaseInstruction
        lblPrimarySuppRep.Text = ChaseDef.EmployeeName

        If ChaseDef.UserCanChange = False Then
            drpIntRepList.Enabled = False
        Else
            drpIntRepList.Enabled = True
        End If
    ...
    Protected Sub ShowEmpListByChase(ByVal ChaseDefinitionID As String, ByVal InterestType As String)
        ...
        drpIntRepList.Items.Add(New ListItem(Session("EmployeeName"), Session("EmployeeID")))
    End Sub
    ...
    Protected Sub btnGenerateChase_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnGenerateChase.Click
        ...
        Try
            ...
            GenChase.ActivationTime = Convert.ToDateTime(txtActivationTime.Text)

            If txtActivationTime.Text = "" Then
                MessageBox("Please Set the Activation Date of the chase.")
                Exit Sub
            End If

            GeneratedChaseID = GenChaseData.fnInsertGenGeneratedChase(GenChase)
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If InStr(MenuIDs, "GenGenerateChase~") = 0 Then
        ...
        If Not IsPostBack Then
            ...
            GetAllULCBranch()
            GetCategoryList()
            txtActivationTime.Text = System.DateTime.Today.ToString("dd-MMM-yyyy")
        Else
            Try
    ...
    Protected Sub drpCategoryList_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles drpCategoryList.SelectedIndexChanged
        ...
        ClearChaseList()
        ShowActiveChaseList(drpCategoryList.SelectedValue, "N\A", Session("EmployeeID"))
    End Sub
    ...
    Protected Sub lnkBtnVeiwContactDetails_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkBtnVeiwContactDetails.Click
        Try
            Dim EmployeeID As String = ""
            If drpRequestFor.SelectedIndex = -1 Then
                EmployeeID = "N\A"
            Else
                EmployeeID = drpRequestFor.SelectedValue
            End If

            Dim lbl As New System.Web.UI.WebControls.Label()
            lbl.Text = "<script language='javascript'>" & Environment.NewLine _
                       & "openWindow('../frmViewContactDetails.aspx?EmployeeID=" & EmployeeID & "','Popup',600,600);</script>"
            Page.Controls.Add(lbl)
        Catch ex As Exception
            MessageBox(ex.Message)
        End Try
